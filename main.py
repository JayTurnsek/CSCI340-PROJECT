...
def handle_predator(ind1, ind2):
    ...
    elif ind1.behavior == 'gb_altruist' and ind2.behavior == 'selfish':
        if random.random() < 0.75:
            ind1.setFoodState(True)
        if random.random() < 0.75:
            ind2.setFoodState(True)
    elif ind1.behavior == 'selfish' and ind2.behavior == 'gb_altruist':
        if random.random() < 0.75:
            ind1.setFoodState(True)
        if random.random() < 0.75:
            ind2.setFoodState(True)
...
def do_generation_gb_selfish(population: list[Individual], predator_rate: float, reprod_rate: float):
    # get randomized population
    shuffled = random.sample(population, len(population))
    ...
    selfish_count, altruist_count = 0, 0
    for individual in population:
        if individual.behavior == 'selfish':
            selfish_count += 1
    ...
    return next_gen, selfish_count/len(next_gen), altruist_count/len(next_gen)
...
def do_simulation_gb_selfish(gens: int, size: int, predator_rate: float, reprod_rate: float):
    ...
    for _ in range(gens):
        ...
        population, selfish_cur, altruist_cur = do_generation_gb_selfish(population, predator_rate, reprod_rate)
        selfish.append(selfish_cur)
        altruist.append(altruist_cur)

    
    return selfish, altruist
...
if __name__ == "__main__":
    ...
    plt.plot(range(N_GENS), metrics, label="Population", color="yellow")
    ...
    plt.plot(range(N_GENS), pop2, label="Selfless Proportion", color="yellow")
    ...
    # GREEN BEARD ALTRUISM VS SELFISH
    pop1, pop2 = do_simulation_gb_selfish(
        N_GENS,
        POP_SIZE,
        PRED_RATE,
        0.6
    ...
    plt.plot(range(N_GENS), pop1, label="Selfish", color="red")
    plt.plot(range(N_GENS), pop2, label="Green Beard Altruists", color="green")
    plt.fill_between(range(N_GENS), pop2, pop1, color="green")
    plt.fill_between(range(N_GENS), pop1, 0, color="red")
    plt.ylim(0, 1.2)
    plt.legend(facecolor='white', framealpha=1)
    plt.title('Population Distribution')
    plt.ylabel('Proportion')
    plt.xlabel('Generation')
    plt.savefig('selfish_gb.png')
